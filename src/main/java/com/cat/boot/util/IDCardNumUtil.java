...
public class IDCardNumUtil {
	 ...
	 *            身份证号
	 * @param version
	 *            old 验证15位或者18位 new 验证18位
	 * @return 有效：true 无效：false
	 */
	public JSONObject IDCardValidate(String IDStr, String version)
		...
		if ("old".equals(version)) {
			if (IDStr.length() != 15 && IDStr.length() != 18) {
				errorInfo = "身份证位数错误,号码长度应该为15位或者18位。";
			}
		} else {
			if (IDStr.length() != 18) {
				errorInfo = "身份证位数错误,号码长度应该为18位。";
			}
		}
		...
		if (isNumeric(Ai) == false) {
			errorInfo = "身份证位数错误,18位号码除最后一位外，都应为数字。";
		}
		// =======================(end)========================

		// ================ 初始化性别 ================
		String strXingBie = Ai.substring(16, 17);
	...
	public static JSONObject validate(String certNumber, String version) {
		IDCardNumUtil cc = new IDCardNumUtil();
		try {
			return cc.IDCardValidate(certNumber, version);
		} catch (ParseException e) {
			JSONObject o = new JSONObject();
			o.put("error", "身份证无效，解析错误！");
			return o;
		}
	}
	...
	public static void main(String[] args) throws ParseException {
		...
		IDCardNumUtil cc = new IDCardNumUtil();
		JSONObject result = cc.IDCardValidate(IDCardNum, "old");

		if (result != null) {
			System.out.println(result);
		} else {
			String bd = result.getString("birthday");
